...
class LegacyInclude < Base
  ...
  def extract_params_hash(context)
    params = {}
    markup = @params
...
class SuperOptimizedInclude < Base
  ...
  def tokenize_params
    @param_tokens = ParameterTokenList.new(@params.scan(PARAMS_PATTERN))
  end
...
class Profiler
  ...
  def debug
    KLASSES.map do |label, klass|
      [label.rjust(20), ": ", klass.new(@markup).extract_params_hash(CONTEXT)].join
    end.join("\n")
  end
